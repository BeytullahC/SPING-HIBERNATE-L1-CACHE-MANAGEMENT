...
public interface UserRepository extends JpaRepository<User, Integer>,
  ...
  /**
   * Redeclaration of {@link CrudRepository#findById(java.io.Serializable)} to change transaction
   * configuration.
   */
  @Transactional
  @Override
  ...
  /**
   * Redeclaration of {@link CrudRepository#deleteById(java.io.Serializable)}. to make sure the
   * transaction configuration of the original method is considered if the redeclaration does not
   * carry a {@link Transactional} annotation.
   */
  @Override
  void deleteById(Integer id); // DATACMNS-649

  /**
   * Retrieve users by their email address. The finder {@literal User.findByEmailAddress} is
   * declared as annotation at {@code User}.
   *
   * @param emailAddress
  ...
  /**
   * Retrieves users by the given email and lastname. Acts as a dummy method declaration to test
   * finder query creation.
   *
   * @param emailAddress
   * @param lastname
  ...
  /**
   * Retrieves users by email address and lastname or firstname. Acts as a dummy method declaration
   * to test finder query creation.
   *
   * @param emailAddress
   * @param lastname
   * @param username
   ...
   */
  List<User> findByEmailAddressAndLastnameOrFirstname(String emailAddress, String lastname,
      String username);
  ...
  /**
   * Method to directly create query from and adding a {@link Pageable} parameter to be regarded on
   * query execution.
   *
   * @param pageable
   * @param lastname
  ...
  /**
   * Method to directly create query from and adding a {@link Pageable} parameter to be regarded on
   * query execution. Just returns the queried {@link Page}'s contents.
   *
   * @param firstname
   * @param pageable
   * @return
  ...
  /**
   * Method where parameters will be applied by name. Note that the order of the parameters is then
   * not crucial anymore.
   *
   * @param foo
   * @param bar
  ...
  void updateUserActiveState(@Param("activeState") boolean activeState,
  ...
  /**
   * Implicitly mapped to a procedure with name "plus1inout" in database via alias. Showing that
   * outputParameterName is ignored when not a NamedStoredProcedure
   */
  @Procedure(procedureName = "plus1inout", outputParameterName = "fakeName")
  // DATAJPA-707
  ...
  /**
   * Explicitly mapped to named stored procedure "User.plus1IO" in {@link EntityManager}. with an
   * invalid outputParameterName - test will fail
   */
  @Procedure(name = "User.plus1IO", outputParameterName = "fakeName")
  // DATAJPA-707
  Integer entityAnnotatedCustomNamedProcedurePlus1IOInvalidOutParamName(@Param("arg") Integer arg);

  /**
   * Explicitly mapped to named stored procedure "User.plus1IO2" in {@link EntityManager}. Stored
   * Proc has 2 out params, but naming one out param here so it only returns one
   */
  @Procedure(name = "User.plus1IO2", outputParameterName = "res2")
  // DATAJPA-707
  Integer entityAnnotatedCustomNamedProcedurePlus1IO2TwoOutParamsButNamingOne(
      @Param("arg") Integer arg);

  /**
   * Explicitly mapped to named stored procedure "User.plus1IO2" in {@link EntityManager}. Returns 2
   * out params as a Map.
   */
  @Procedure(name = "User.plus1IO2")
  // DATAJPA-707 DATAJPA-1579
   ...
   */
  @Procedure(name = "User.plus1IOoptional")
  // DATAJPA-1579
  Map<String, Integer> entityAnnotatedCustomNamedProcedurePlus1IOoptional(
      @Param("arg") Integer arg);
  ...
  List<User> findUsersByFirstnameForSpELExpressionWithParameterVariableOnly(
      @Param("firstname") String firstname);
  ...
  List<User> findUsersByFirstnameAsStringAndUserLastnameAsSpELExpression(
      @Param("firstname") String firstname,
      @Param("user") User user);

  // DATAJPA-1140
  @Query("select u from User u where u.firstname =:#{#user.firstname} and u.lastname =:#{#lastname}")
  List<User> findUsersByUserFirstnameAsSpELExpressionAndLastnameAsFakeSpELExpression(
      @Param("user") User user,
      @Param("lastname") String lastname);
  ...
  List<User> findUsersByFirstnameForSpELExpressionWithParameterIndexOnlyWithEntityExpression(
      String firstname,
      String lastname);
  ...
  List<String> findAllAndSortByFunctionResultNamedParameter(
      @Param("namedParameter1") String namedParameter1,
  ...
  @Query(
      value = "merge into sd_user " + "using (select id from sd_user where age < 30) request "
          + "on (sd_user.id = request.id) " + "when matched then "
